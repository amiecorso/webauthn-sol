...
contract ProfileOne is Test {
    ...
    function setUp() public {
        ...
        // Etch Solady P256 verifier runtime at the expected fallback address (0x26).
        bytes memory soladyVerifier =
        ...
        vm.etch(address(0x26), soladyVerifier);
    }
    ...
    function test_profileIndex() public {
        ...
        uint256 g0 = gasleft();
        (bool succ, bytes memory ret) = address(this).call(abi.encodeWithSelector(this._verifyWrapper.selector, challenge, auth, x, y));
        ...
        console2.log("OK:");
        console2.logUint(ok ? 1 : 0);
        console2.log("GAS:");
        console2.logUint(used);
        console2.log("MSGHASH:");
        console2.logBytes32(msgHash);
        console2.log("R:");
        console2.logUint(auth.r);
        console2.log("S:");
        console2.logUint(auth.s);
        console2.log("X:");
        console2.logUint(x);
        console2.log("Y:");
        console2.logUint(y);
    ...
    function _verifyWrapper(bytes memory challenge, WebAuthn.WebAuthnAuth memory auth, uint256 x, uint256 y) external returns (bool) {
        return WebAuthn.verify(challenge, false, auth, x, y);
    }

    // Just used to figure out the correct argument order for the Solady verifier.
    function test_directSoladyCall() public {
        uint256 idx = 0;
        string memory root = vm.projectRoot();
        string memory path = string.concat(root, IN_FILE);
        string memory json = vm.readFile(path);
        ...
        bytes memory a1 = abi.encode(msgHash, auth.r, auth.s, x, y);
        (bool s1, bytes memory o1) = address(0x26).staticcall(a1);
        console2.log("ORDER h,r,s,x,y:");
        console2.logUint(s1 && o1.length >= 32 ? abi.decode(o1, (uint256)) : 999);

        bytes memory a2 = abi.encode(auth.r, auth.s, x, y, msgHash);
        (bool s2, bytes memory o2) = address(0x26).staticcall(a2);
        console2.log("ORDER r,s,x,y,h:");
        console2.logUint(s2 && o2.length >= 32 ? abi.decode(o2, (uint256)) : 999);
    }
}
